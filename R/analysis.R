...
pretest <- read_excel("~/GDrive/research/yourfeed_analysis/data/20210129 pretest.xlsx")
...
to_plot<- yourfeed %>%filter(attend1==1) %>% group_by(concordant,real) %>% dplyr::summarize(
...
order <- yf %>% group_by(order) %>% dplyr::summarize(
  y1 = mean(attend1, na.rm=T), 
  ci1 = mean_error(attend1),
  y2 = mean(attend2, na.rm=T), 
  ci2 = mean_error(attend2),
  y3 = mean(attend3, na.rm=T), 
  ci3 = mean_error(attend3)
)
p <- plot(order$order, order$y1, xlim=c(0,140),ylim=c(0,1), pch=21, bg='yellow', xlab = "Item order", ylab="Likelihood of attending")
arrows(x0=order$order,y0=order$y1-order$ci1, y1=order$y1+order$ci1, angle=90, code=3, length=0.05)
points(order$order, order$y1, bg='yellow', pch=21)

arrows(x0=order$order,y0=order$y2-order$ci2, y1=order$y2+order$ci2, angle=90, code=3, length=0.05)
points(order$order, order$y2, bg='green', pch=21)

arrows(x0=order$order,y0=order$y3-order$ci3, y1=order$y3+order$ci3, angle=90, code=3, length=0.05)
points(order$order, order$y3, bg='red', pch=21)
``